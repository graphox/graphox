...
VARFP(theme, 0, 1, 3, themereload());

Texture *texload(const char* name, int clamp) {
	Texture *t;
	if(!fileexists(name,"") && !fileexists( path(name,true),"")) {
		if(theme == 0)
			defformatstring(p)("data/themes/default/%s", name);
		if(theme == 1)
			defformatstring(p)("data/themes/se/%s", name);
		if(theme == 2)
