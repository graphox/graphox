...
struct gui : g3d_gui
{
    ...
    void previewslot(VSlot &vslot, bool overlaid, int x, int y, int size, bool hit)
    {
        ...
        {
			if(theme == 0)
				if(!overlaytex) overlaytex = textureload("data/themes/default/guioverlay.png", 3);
			if(theme == 1)
				if(!overlaytex) overlaytex = textureload("data/themes/se/guioverlay.png", 3);
			if(theme == 2)
				if(!overlaytex) overlaytex = textureload("data/themes/crash/guioverlay.png", 3);
			if(theme == 3)
				if(!overlaytex) overlaytex = textureload("data/themes/red eclipse/guioverlay.png", 3);
            glBindTexture(GL_TEXTURE_2D, overlaytex->id);
            glColor3fv(light.v);
            rect_(x, y, xs, ys, 0);
        }
    ...
    void line_(int size, float percent = 1.0f)
    {
        ...
        {
			if(theme == 0)
				if(!slidertex) slidertex = textureload("data/themes/default/guislider.png", 3);
			if(theme == 1)
				if(!slidertex) slidertex = textureload("data/themes/se/guislider.png", 3);
			if(theme == 2)
				if(!slidertex) slidertex = textureload("data/themes/crash/guislider.png", 3);
			if(theme == 3)
				if(!slidertex) slidertex = textureload("data/themes/red eclipse/guislider.png", 3);
            glEnable(GL_TEXTURE_2D);
        ...
        if(visible())
        {
            ...
            if(icon)
            {
                if(!isspace(icon[0]))
                {
                    ...
                    if(theme == 3)
                    {
                        defformatstring(tname)("data/themes/red eclipse/icons/%s%s", icon, ext ? "" : ".jpg");
    ...
    static Texture *skintex, *overlaytex, *slidertex, *cursortex;
    ...
    {
		if(theme == 0)
			if(!skintex) skintex = textureload("data/themes/default/guiskin.png", 3);
		if(theme == 1)
			if(!skintex) skintex = textureload("data/themes/se/guiskin.png", 3);
		if(theme == 2)
			if(!skintex) skintex = textureload("data/themes/crash/guiskin.png", 3);
		if(theme == 3)
			if(!skintex) skintex = textureload("data/themes/red eclipse/guiskin.png", 3);
