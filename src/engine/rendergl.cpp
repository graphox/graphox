...
VARP(_Graphox_fullscreenzoomcrosshair, 0, 0, 1);
...
void drawcrosshair(int w, int h)
{
    ...
    if(windowhit)
    {
        ...
        {
            if(!gui::cursortex) gui::cursortex = textureload("data/guicursor.png", 3, true);
        }

        crosshair = gui::cursortex;
        if(theme == 3)
            chsize = cursorsize*w/1800.0f;
        else
	...
	if(_Graphox_fullscreenzoomcrosshair == 1) {
		if(!zoom || windowhit) {
...
void gl_drawhud(int w, int h)
{
	...
	int textoff = 1;      if(minimapon) textoff = 16;      else textoff = 1;
