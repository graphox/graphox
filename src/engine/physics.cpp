...
#ifdef _GRAPHOX_
	VAR(_Graphox_jump_multipier, 0, 1, 100);
	VAR(_Graphox_gravity_multipier, 0, 1, 100);
#endif
...
bool bounce(physent *d, float secs, float elasticity, float waterfric)
{
    ...
    if(water)
    {
    	...
    	#else
    		d->vel.z -= (GRAVITY*_Graphox_gravity_multipier)/16*secs;
    	#endif
    ...
    #else
    	else d->vel.z -= GRAVITY*secs*_Graphox_gravity_multipier;
    #endif
    vec old(d->o);
    loopi(2)
    {
...
void modifyvelocity(physent *pl, bool local, bool water, bool floating, int curtime)
{
    if(floating)
    {
        if(pl->jumping)
        {
            ...
            #else
				pl->vel.z = max(pl->vel.z, (JUMPVEL * _Graphox_jump_multipier));
            #endif
        }
    }
...
void modifygravity(physent *pl, bool water, int curtime)
{
    ...
    #ifdef _GRAPHOX_
    	secs *= _Graphox_gravity_multipier;
    #endif
    vec g(0, 0, 0);
