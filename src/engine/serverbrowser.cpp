...
#include "SDL_thread.h"
	...
	#else
		//omg that you didn't knew this
		#include "game.h"
    #endif
	#if 0
		#ifdef _LINUX_
		    #include "game.h"//a_teammate: mastermodesorting 06.03.2011
		#endif
...
char* klbuchstaben(char* convertstring)
{
	for(int position = 0; position < sizeof(convertstring); position++){
		if(convertstring[position] >= 65 && convertstring[position] <= 90 && convertstring[position] != 0)
			convertstring[position] = convertstring[position]+32;
		else convertstring[position] = convertstring[position];
	}
...
struct serverinfo
{
    ...
    static int compare(serverinfo **ap, serverinfo **bp) //a_teammate 26.02.2011
    {
        ...
        /*if(strcmp(searchstr, "")) {
			...
			char *bdesc;
			adesc = a->sdesc;
			adesc = klbuchstaben(adesc);
			bdesc = b->sdesc;
			bdesc = klbuchstaben(bdesc);
			//wandelt in klbuchstaben um
			if(strstr(adesc, searchstr) < strstr(bdesc, searchstr)) return 1;
			if(strstr(adesc, searchstr) > strstr(bdesc, searchstr)) return -1;
		}*/
