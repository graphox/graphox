...
//            particle_fireball(b.ammopos, 4.8f, PART_EXPLOSION, 0, b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 4.8f);
    ...
    void drawhud(fpsent *d, int w, int h)
    {
        ...
        else if(game::radartheme != 1)
        {
			settexture("packages/hud/blip_blue.png", 3);
		}
		else
		{
			settexture("data/themes/hud/blip_blue.png", 3);
		}
        glPushMatrix();
        ...
        if(basenumbers) setfont("digit_grey");
        	...
        	settexture("packages/hud/blip_grey.png", 3);
        else
        	settexture("data/themes/hud/blip_grey.png", 3);
        drawblips(d, blipsize, fw, fh, 0, showenemies);
        if(basenumbers) setfont("digit_red");
        else if(game::radartheme != 1)
        	settexture("packages/hud/blip_red.png", 3);
        else
        	settexture("data/themes/hud/blip_red.png", 3);
        drawblips(d, blipsize, fw, fh, -1, showenemies);
