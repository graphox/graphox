...
namespace server
{
    ...
    struct user
    {
    	const char *password;
    	int priv;
    	user (const char *pass_, int priv_) : password(pass_), priv(priv_) {};
    };
    
    vector<user *> users;
	
	void adduser(const char *pass, const char *priv)
	{
		int priv_ = 0;
		
		if(strcmp(priv, "admin") == 0)
			priv_ = 2;
		else if(strcmp(priv, "master") == 0)
			priv_ = 1;

		
		user *u = new user(pass, priv_);
		users.add(u);
		conoutf("adding user: %s %s", pass, priv);
	}
	ICOMMAND(user, "ss", (char *s, char *priv), adduser(s, priv));
	COMMAND(adduser, "ss");
    ...
    void setmaster(clientinfo *ci, bool val, const char *pass = "", const char *authname = NULL)
    {
		...
		sendf(ci->clientnum, 1, "ris", N_SERVMSG, "checking your password.");
       	loopv(users)
       	{
       		...
       		if(checkpassword(ci, users[i]->password, pass))
       		{
       			sendf(ci->clientnum, 1, "ris", N_SERVMSG, "You successfully authed.");
       			ci->privilege = users[i]->priv;
       			return;
       		};
       	};
        if(val)
