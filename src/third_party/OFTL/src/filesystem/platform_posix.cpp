...
namespace filesystem
{
    ...
    void File_Info::init_from_str(const char *path)
    {
        ...
        if (S_ISREG(info.st_mode))
            p_type = OFTL_FILE_REG;
        else if (S_ISDIR(info.st_mode))
            p_type = OFTL_FILE_DIR;
        else if (S_ISCHR(info.st_mode))
            p_type = OFTL_FILE_CHR;
        else if (S_ISBLK(info.st_mode))
            p_type = OFTL_FILE_BLK;
        else if (S_ISFIFO(info.st_mode))
            p_type = OFTL_FILE_FIFO;
        else if (S_ISLNK(info.st_mode))
            p_type = OFTL_FILE_LNK;
        else if (S_ISSOCK(info.st_mode))
            p_type = OFTL_FILE_SOCK;
        else
            p_type = OFTL_FILE_UNKNOWN;
