...
#ifndef OFTL_FILESYSTEM_H
#define OFTL_FILESYSTEM_H
...
namespace filesystem
{
     ...
     *
     * Types:
     *  OFTL_FILE_UNKNOWN - unknown file type.
     *  OFTL_FILE_FIFO - named pipe (FIFO) type (POSIX only).
     *  OFTL_FILE_CHR - character device type (POSIX only).
     *  OFTL_FILE_DIR - directory type.
     *  OFTL_FILE_BLK - block device type (POSIX only).
     *  OFTL_FILE_REG - regular file type.
     *  OFTL_FILE_LNK - symbolic link type.
     *  OFTL_FILE_SOCK - UNIX domain socket type (POSIX only).
     *  OFTL_FILE_WHT - whiteout file type (POSIX only).
     */
    enum File_Type
    {
        OFTL_FILE_UNKNOWN = 0,
    ...
    /* Function: join_path
     * Joins a set of strings (<String>, const char*, char) into a path
     * and returns a <File_Info> of the path. Example:
     *
     * (start code)
     ...
     * (end)
     */
    template<typename... T> inline File_Info join_path(const T&... args);
    ...
    struct File_Info
    {
        ...
        File_Info():
            ...
            p_type(OFTL_FILE_UNKNOWN), p_path(types::String()),
    ...
    template<size_t i> struct Join_Path_Impl
    {
        template<typename T, typename... U>
        static void join(types::String& s, const T& a, const U&... b)
        {
            s += a; s += separator();
            Join_Path_Impl<i - 1>::join(s, b...);
        }
    };

    template<> struct Join_Path_Impl<1>
    {
        template<typename T>
        static void join(types::String& s, const T& a)
        {
            s += a;
        }
    };
    ...
    template<typename... T>
    inline File_Info join_path(const T&... args)
    {
        types::String path;
        Join_Path_Impl<sizeof...(T)>::join(path, args...);
